...
export function TurnCard({
  playerName, round, total,
}: { playerName: string; round: Round; total: number }) {
  const miss = missOutcome(total);

  return (
    <div className="flex w-full shrink-0 flex-col gap-3.5 rounded-xl border border-line bg-surface p-5 lg:w-80">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-[13px] leading-relaxed text-ink-2">{instructionFor(round)}</p>
      </div>
...
function instructionFor(round: Round): string {
  switch (round.kind) {
    case 'sum':
      return `Tap every number you hit as a ${round.key === 'D' ? 'double' : 'treble'}. The pad adds them up and applies the multiplier.`;
    case 'binary':
      return 'Three darts totalling exactly 41. All or nothing — and a miss halves everything above.';
    case 'bull':
      return 'Count the bull units. Outer bull is one, bullseye is two.';
    default:
      return `How many ${round.label}s did the three darts land? A treble counts three, a double two.`;
  }
}
